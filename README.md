...
- `DateTimePopulator`. Sets `date_time` in the format `YYYY-mm-dd HH:MM:SS.eee[+/-]HH:MM`.
...
the destructor to be `noexcept(false)` so that exceptions may be thrown from it.
Of course, we make sure to catch any exceptions and manually call the Executor
members' destructors before rethrowing.
